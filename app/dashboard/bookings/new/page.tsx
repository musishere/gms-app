...
import { BookMarked, MapPin, ChevronRight, CheckCircle2, Loader2, AlertCircle, ArrowLeft } from 'lucide-react';
import Link from 'next/link';

interface Grave { id: string; graveNumber: string; section: string; size: string; price: number; status: string; row: number; column: number; }
...
export default function NewBookingPage() {
  ...
  const [step, setStep] = useState<1 | 2>(preGraveId ? 2 : 1);
  ...
  useEffect(() => {
    setLoading(true);
    setFetchError('');
    fetch(`/api/graves?section=${activeSection}`)
      .then(async r => {
        ...
        if (preGraveId && !selectedGrave) {
          ...
          if (found && found.status === 'available') setSelectedGrave(found);
        }
      })
  ...
  const submit = async () => {
    ...
    try {
      const r = await fetch('/api/bookings', {
        ...
        body: JSON.stringify({ graveId: selectedGrave.id, ...form }),
      });
      const d = await r.json();
        ...
        <div className="flex items-start gap-3 bg-yellow-500/10 border border-yellow-500/30 rounded-xl px-4 py-3">
          ...
          <div className="text-sm">
            ...
            <p className="text-yellow-400/70 text-xs mt-0.5">
              No graves have been seeded yet. Go to <Link href="/dashboard/graves" className="underline hover:text-yellow-300">Grave Map</Link> — if it also shows nothing, call <code className="bg-yellow-500/10 px-1 rounded">POST /api/setup/graves</code> with your service role key to seed 390 graves.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        {[1, 2].map(s => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border transition ${step >= s ? 'bg-emerald-500 border-emerald-400 text-white' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>{s}</div>
            <span className={`text-xs font-medium ${step >= s ? 'text-white' : 'text-slate-500'}`}>{s === 1 ? 'Select Grave' : 'Booking Details'}</span>
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {SECTIONS.map(s => (
              <button key={s} onClick={() => setActiveSection(s)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition ${activeSection === s ? 'bg-emerald-500 text-white' : 'bg-slate-900 border border-slate-700 text-slate-400 hover:text-white'}`}>
                Section {s}
              </button>
            ))}
                ...
                {dbTotal === 0 ? (
                  <>
                    ...
                    <p className="text-xs text-center text-slate-500 max-w-xs">
                      The graves table is empty. Run <code className="bg-slate-800 px-1.5 py-0.5 rounded text-emerald-300">POST /api/setup/graves</code> with your service role key to seed 390 graves, then refresh.
                    </p>
                  </>
                  ...
                  <>
                    ...
                    <p className="text-xs text-slate-600">Section {activeSection} has {totalInSection} graves — try a different size</p>
                  ...
                  <>
                    <p className="text-sm">No graves in Section {activeSection}</p>
                    <p className="text-xs text-slate-600">Try a different section</p>
                  </>
                )}
              </div>
            ) : (
          ...
          <div className="flex justify-end">
            <button onClick={() => setStep(2)} disabled={!selectedGrave}
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-5">
            ...
            <div className="flex gap-3 justify-between">
              <button onClick={() => { if (!preGraveId) setStep(1); }} disabled={!!preGraveId}
              ...
              <button onClick={submit} disabled={submitting || !form.deceasedName || !form.contactName || !form.contactPhone}
          ...
          <div>
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4 sticky top-6">
              <h2 className="text-sm font-semibold text-white">Selected Grave</h2>
              {selectedGrave ? (
                <div className="space-y-3">
                  ...
                  {!preGraveId && (
                    <button onClick={() => { setSelectedGrave(null); setStep(1); }}
                ...
                <p className="text-sm text-slate-500">No grave selected — click an available slot in the grid</p>
              )}
