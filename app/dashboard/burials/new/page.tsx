...
import { Loader2, Zap, MapPin, CheckCircle2, AlertCircle } from 'lucide-react';

interface Grave { id: string; graveNumber: string; section: string; size: string; price: number; status: string; latitude?: number; longitude?: number; }
...
export default function NewBurialPage() {
  ...
  const [form, setForm] = useState({
    deceasedName: '', deceasedCNIC: '', dateOfBirth: '', dateOfDeath: '', causeOfDeath: '',
  ...
  useEffect(() => {
    fetch('/api/graves?status=available').then(r => r.json()).then(d => {
      setGraves(d.graves ?? []);
      if (preGraveId) {
        const g = (d.graves ?? []).find((g: Grave) => g.id === preGraveId);
  ...
  const autoAllocate = async () => {
    ...
    const r = await fetch('/api/graves/allocate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ size: form.graveSize, section: form.graveSection || undefined }) });
    const d = await r.json();
    if (d.grave) setSelectedGrave(d.grave); else setError(d.error || 'No available graves');
    setAllocating(false);
  };
    ...
    try {
      const r = await fetch('/api/burials', {
        ...
        body: JSON.stringify({
          ...
          deceased: { name: form.deceasedName, cnic: form.deceasedCNIC, dateOfBirth: form.dateOfBirth, dateOfDeath: form.dateOfDeath, causeOfDeath: form.causeOfDeath, religion: form.religion, nationality: form.nationality, address: form.address, nextOfKin: form.nextOfKin, nextOfKinPhone: form.nextOfKinPhone, nextOfKinCNIC: form.nextOfKinCNIC, relationship: form.relationship },
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Input form={form} onChange={h} name="deceasedCNIC" label="CNIC (if available)" placeholder="00000-0000000-0" />
            ...
            <div className="flex justify-end mt-4">
              <button onClick={() => { if (!form.deceasedName || !form.dateOfDeath || !form.nextOfKin || !form.nextOfKinPhone) { setError('Fill all required fields'); return; } setError(''); setStep(2); }}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
              ...
              <div className="flex items-end">
                <button onClick={autoAllocate} disabled={allocating} className="w-full flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-400 disabled:opacity-50 text-white font-semibold py-2 rounded-lg text-sm transition">
              ...
              <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-400/70 mt-0.5">Section {selectedGrave.section} · {selectedGrave.size} · {formatCurrency(selectedGrave.price)}{selectedGrave.latitude ? ` · GPS: ${selectedGrave.latitude.toFixed(4)}, ${selectedGrave.longitude?.toFixed(4)}` : ''}</p>
                  </div>
                  <button onClick={() => setSelectedGrave(null)} className="ml-auto text-slate-400 hover:text-white text-xs">Change</button>
                </div>
              </div>
            ) : (
          ...
          <div className="space-y-4">
            ...
            {selectedGrave && (
              <div className="bg-slate-800 rounded-xl p-4 mt-2">
                <p className="text-xs text-slate-400 mb-2 font-medium">Summary</p>
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between font-semibold"><span className="text-slate-400">Burial Fee</span><span className="text-emerald-400">{formatCurrency(selectedGrave.price)}</span></div>
                </div>
              </div>
            )}
            <div className="flex justify-between mt-4">
              ...
              <button onClick={submit} disabled={submitting} className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-white font-semibold px-6 py-2.5 rounded-xl text-sm transition">
