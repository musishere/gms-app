...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (auth.role === 'family') {
      ...
      if (b.status !== 'pending') return NextResponse.json({ error: 'Only pending bookings can be cancelled' }, { status: 409 });
    }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!['pending', 'cancelled'].includes(String(b.status)))
      return NextResponse.json({ error: 'Cannot delete an approved or converted booking' }, { status: 409 });

    await admin.from('grave_bookings').delete().eq('id', id);
    return NextResponse.json({ success: true });
  } catch (e) { return errorResponse('Failed to delete booking', e); }
}
