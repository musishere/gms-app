...
const F = ({ name, label, type = 'text', placeholder, req = false, value, onChange }: any) => (
  <div>
    <label className="block text-sm font-medium text-slate-300 mb-1.5">{label}{req && <span className="text-red-400">*</span>}</label>
    <input type={type} name={name} value={value} onChange={onChange} required={req} placeholder={placeholder}
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
          <form onSubmit={submit} className="space-y-4">
            ...
            <F name="name" label="Full Name " req placeholder="Muhammad Ahmad" value={form.name} onChange={h} />
            <F name="email" label="Email " type="email" req placeholder="email@example.com" value={form.email} onChange={h} />
            <F name="password" label="Password " type="password" req placeholder="Min. 8 characters" value={form.password} onChange={h} />
            <F name="phone" label="Phone Number" placeholder="+92 300 0000000" value={form.phone} onChange={h} />
            <F name="cnic" label="CNIC" placeholder="00000-0000000-0" value={form.cnic} onChange={h} />
            <F name="address" label="Address" placeholder="House #, Street, City" value={form.address} onChange={h} />
